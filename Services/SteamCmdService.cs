...
namespace SteamCmdWebAPI.Services
{
    public class SteamCmdService
    {
        ...
        {
            // Đảm bảo chỉ một tiến trình được xử lý tại một thời điểm
            var cancellationToken = new CancellationTokenSource(TimeSpan.FromMinutes(30)).Token;

            // Dừng tiến trình SteamCMD hiện tại của profile này (nếu có)
            if (_steamCmdProcesses.TryRemove(profileId, out var existingProcess))
            {
                try
                {
                    if (existingProcess != null && !existingProcess.HasExited)
                    {
                        ...
                        if (!existingProcess.WaitForExit(10000)) // 10 giây timeout
                        {
                            _logger.LogWarning("Không thể dừng tiến trình SteamCMD cũ cho profile {ProfileId}", profileId);
                        }
                    }
                }
                catch (Exception ex)
            ...
            try
            {
                ...
                // Cập nhật trạng thái profile sau khi chạy xong
            ...
            catch (Exception ex)
            {
                ...
                // Đảm bảo cập nhật trạng thái profile thành Stopped nếu có lỗi
                profile.Status = "Stopped";
                profile.StopTime = DateTime.Now;
                profile.Pid = 0;
                await _profileService.UpdateProfile(profile);
        ...
        public Task<bool> IsSteamCmdInstalled() // Bỏ async vì không cần await
        {
        ...
        public async Task InstallSteamCmd()
        {
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    await SafeSendLogAsync("System", "Info", "Đang tải SteamCMD...");
                    ...
                    await SafeSendLogAsync("System", "Success", "Đã tải xong SteamCMD");
                    _logger.LogInformation("Đã tải xong SteamCMD vào {Path}", zipPath);
                }

                await SafeSendLogAsync("System", "Info", "Đang giải nén...");
                _logger.LogInformation("Bắt đầu giải nén SteamCMD...");
                ...
                try
                {
                    ...
                    await SafeSendLogAsync("System", "Info", $"Đã xóa file zip: {zipPath}");
                }
                catch (Exception ex)
                {
            ...
            {
                _logger.LogError(ex, "Lỗi trong quá trình cài đặt SteamCMD");
        ...
        private string GetSteamCmdPath()
        {
            ...
            return Path.Combine(steamCmdDir, executable);
        }
        ...
        private async Task CreateSteamAppsSymLink(string gameInstallDir)
        {
            ...
            try
            {
                ...
                // Xóa thư mục steamapps cục bộ nếu đã tồn tại
                if (Directory.Exists(localSteamAppsPath))
                {
                    try
                    {
                        ...
                        await SafeSendLogAsync("System", "Info", $"Đã xóa thư mục steamapps cục bộ tại {localSteamAppsPath}");
                    }
                    catch (UnauthorizedAccessException)
                    {
                        _logger.LogWarning("Không thể xóa {Path}, có thể là symbolic link", localSteamAppsPath);
                        // Xử lý riêng cho symbolic link đã tồn tại
                        if (OperatingSystem.IsWindows())
                        {
                ...
                // Sử dụng .NET Core APIs cho symbolic link khi có thể
                if (OperatingSystem.IsWindows())
                {
                    try
                    {
                        ...
                        {
                            _logger.LogInformation("Đã tạo symbolic link từ {Source} đến {Target}", gameSteamAppsPath, localSteamAppsPath);
                    ...
                    {
                        // Dự phòng nếu API không hoạt động, sử dụng cmd
                        _logger.LogWarning("Không thể sử dụng Directory.CreateSymbolicLink, thử dùng cmd");
                    }
                }

                // Tạo symbolic link bằng process - phương pháp dự phòng
                bool success = await CreateSymbolicLinkWithProcess(gameSteamAppsPath, localSteamAppsPath);
                if (!success)
                {
        ...
        private async Task RemoveSteamAppsSymLink()
        {
            ...
            try
            {
                if (Directory.Exists(localSteamAppsPath))
                {
                    try
                    {
                        Directory.Delete(localSteamAppsPath, false); // Chỉ xóa symbolic link, không xóa nội dung
        ...
        private async Task RunSteamCmdAsync(string steamCmdPath, SteamCmdProfile profile, int profileId)
        {
            ...
            finally
            {
                ...
                if (_steamCmdProcesses.TryRemove(profileId, out var removedProcess))
                {
                    try
                    {
                        if (removedProcess != null && !removedProcess.HasExited)
                        {
                            removedProcess.Kill();
                            removedProcess.WaitForExit(10000); // 10 giây timeout
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public async Task<bool> RestartProfileAsync(int profileId)
        {
            ...
            if (_steamCmdProcesses.TryGetValue(profileId, out var process))
            {
                try
                {
                    ...
                    {
                        process.Kill();
                        process.WaitForExit(10000); // 10 giây timeout
                    }
                }
                catch (Exception ex)
                {
            ...
            await Task.Delay(2000); // Đợi một chút để đảm bảo process đã dừng hoàn toàn
